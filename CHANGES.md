...
| v0.8.0 | Split `composes` into a protocol-neutral routing core plus per-protocol wrappers. The core keeps strategies, the health state machine, recovery probes, aliases, sticky routing, observers, `Stats()` and `MultiError` attribution, and now imports no provider at all; every OpenAI-wire symbol moves to `composes/openais` (breaking, no deprecation window, no aliases). New: multi-backend **OpenAI Responses** dispatch (`Responses` / `ResponsesStream`) and the new `composes/anthropics` package for **Anthropic Messages** — three dispatch paths over one implementation. Pools still never mix protocols. No provider code changed. Migration table: [MIGRATION.md](./MIGRATION.md); reasoning: [ADR 0008](./doc/adr/0008-shared-routing-core-across-protocol-wrappers.md). |
