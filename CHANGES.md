...
| Version | Change |
|---|---|
| v0.6.1 | Deprecate the canonical API. Every symbol removed in v0.7.0 — package `ais`, the root canonical client / stream / interception / `Responder` surface, and both providers' registry and extension entry points — now carries a `Deprecated:` comment naming its native counterpart. No behavior, signature or serialization change. |
| v0.7.0 | Remove the canonical API. `provider/openai` and `provider/anthropic` are the only public interface, each expressing its protocol completely and independently; `composes` narrows to dispatch within the OpenAI-compatible wire format. Both `HTTPError` types implement `StatusCode() int` (the field is renamed `Status`), both native streams accumulate and report usage, and `openai.ChatCompletionRequest.ExtraBody` carries backend-private parameters. Reasoning: [ADR 0007](./doc/adr/0007-provider-native-as-the-only-public-interface.md). |
| v0.8.0 | Split `composes` into a protocol-neutral routing core plus per-protocol wrappers. The core keeps strategies, the health state machine, recovery probes, aliases, sticky routing, observers, `Stats()` and `MultiError` attribution, and now imports no provider at all; every OpenAI-wire symbol moves to `composes/openais` (breaking, no deprecation window, no aliases). New: multi-backend **OpenAI Responses** dispatch (`Responses` / `ResponsesStream`) and the new `composes/anthropics` package for **Anthropic Messages** — three dispatch paths over one implementation. Pools still never mix protocols. No provider code changed. Reasoning: [ADR 0008](./doc/adr/0008-shared-routing-core-across-protocol-wrappers.md). |
| v0.9.0 | Make `composes` routing stateful. A pool now serves its calls from a single **active endpoint**, chosen by the strategy only when there is none or the current one is judged dead — so successive calls stay on one backend under `StrategyRandom` and `StrategyWeight` too, and a pool stops being a load balancer. A failing endpoint is retried in place (`base × 2^(k-1)`, `WithRetryPolicy`) before being marked dead, and a dead one rejoins the candidates after `WithRecoverTime` without displacing the incumbent. Health narrows to `available` / `dead`; 401/403 skips the retries and kills the endpoint at once, everything else (429, 4xx, 5xx, transport) retries first. Removed with no aliases: `StrategySticky`, `WithSessionID`, `WithStickyFallback`, `WithRecoveryInterval`, `WithCoolingInterval` and the recovery probes. `EndpointStat` gains `Active`. Both wrappers keep their method sets unchanged; no provider code changed. Reasoning: [ADR 0009](./doc/adr/0009-stateful-active-endpoint-with-in-call-retry.md). |
