...
func NewFromEndpoints(
	...
	if len(specs) == 0 {
		return nil, fmt.Errorf("aimodel/composes/openais: at least one endpoint spec is required")
	}
	...
	for i, s := range specs {
		if s.Alias == "" {
			return nil, fmt.Errorf("aimodel/composes/openais: endpoint %d: alias is required", i)
		}

		if prev, dup := seen[s.Alias]; dup {
			return nil, fmt.Errorf("aimodel/composes/openais: duplicate alias %q at endpoints %d and %d", s.Alias, prev, i)
		}
