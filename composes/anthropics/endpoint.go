...
type ModelEntry struct {
	...
	Client Messenger
	// Weight is used by composes.StrategyWeight. Zero is treated as 1.
	Weight int
	...
	// Cost optionally declares static pricing for composes.StrategyCost. Nil
	// sorts after priced endpoints.
	Cost *composes.EndpointCost

	// Latency optionally declares a routing latency for composes.StrategyLatency.
...
type EndpointSpec struct {
	...
	Capability *Capability
	// Cost optionally declares static pricing for composes.StrategyCost.
	Cost *composes.EndpointCost
	// Latency optionally declares a routing latency for composes.StrategyLatency.
	Latency *time.Duration
}
...
func NewFromEndpoints(
	strategy composes.Strategy, specs []EndpointSpec, opts ...composes.Option,
) (*ComposeClient, error) {
	if len(specs) == 0 {
		return nil, fmt.Errorf("aimodel/composes/anthropics: at least one endpoint spec is required")
	}
	...
	for i, s := range specs {
		if s.Alias == "" {
			return nil, fmt.Errorf("aimodel/composes/anthropics: endpoint %d: alias is required", i)
		}

		if prev, dup := seen[s.Alias]; dup {
			return nil, fmt.Errorf("aimodel/composes/anthropics: duplicate alias %q at endpoints %d and %d", s.Alias, prev, i)
		}
