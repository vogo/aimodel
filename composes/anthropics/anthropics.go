...
// this package binds it to `provider/anthropic` types.
//
...
func NewComposeClient(
	...
	if len(entries) == 0 {
		return nil, fmt.Errorf("aimodel/composes/anthropics: at least one model entry is required")
	}

	for i, e := range entries {
		if e.Client == nil {
			return nil, fmt.Errorf("aimodel/composes/anthropics: entry %d (%q): client is nil", i, e.Name)
		}
	}
...
func (c *ComposeClient) Messages(
	...
	return composes.Dispatch(ctx, c.router, messagesCall(request, false),
...
func (c *ComposeClient) MessagesStream(
	...
	return composes.Dispatch(ctx, c.router, messagesCall(request, true),
...
func messagesCall(request *anthropic.MessagesRequest, stream bool) composes.Call {
