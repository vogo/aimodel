...
- Nested protocol details are promoted to the top-level canonical fields: `prompt_tokens_details.cached_tokens` → `CacheReadTokens`; `completion_tokens_details.reasoning_tokens` (OpenAI) / `output_tokens_details.thinking_tokens` (Anthropic) → `ReasoningTokens`. On a unified-client call the promotion happens in each provider's usage translation, reading that provider's native usage type. `Usage.UnmarshalJSON` performs the same promotion for callers decoding canonical JSON directly, under the rule **an explicit top-level field wins** — the nested value is only used when the top-level one is 0.
