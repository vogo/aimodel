...
A new interaction form gets its own methods rather than widening the existing ones — the small-interface rule of [ADR 0002](../adr/0002-provider-native-as-the-only-public-interface.md) §6. Up to v0.5.x this operation was also reachable through a root `Responder` capability on the unified client; that client is gone, and the types it spoke were already these ones, so the migration is to call the same methods on `*openai.Client` directly.
