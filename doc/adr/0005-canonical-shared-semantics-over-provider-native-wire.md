...
- **The canonical↔wire seam is now hand-written on both paths.** Struct embedding no longer passes unknown canonical fields through automatically: a new canonical field that is not wired into `toOpenAIRequest` / `fromOpenAIResponse` is dropped silently, with no compiler error. The mitigation is procedural, not structural — the four-way sync in Architecture §6 requires the native layer and the canonical translation to be updated together, and each protocol's mapping-boundary section records what is intentionally absent.
- **No field-reconciliation guard is added.** A reflective test asserting that every canonical field round-trips through the OpenAI translation was considered and rejected: canonical and native are deliberately *not* isomorphic contracts, so an unmapped field is as likely to be an intended boundary (Architecture §2 lists several) as an oversight. Such a test would encode "full coverage" as the correctness standard, which contradicts the admission rule above, and would need a per-field allowlist that drifts on its own. Reviewers enforce the seam instead.
