...
func TestNativeMessagesRequestAndImmutability(t *testing.T) {
	...
	defer s.Close()
	req := &MessagesRequest{Model: "claude", MaxTokens: 0, Messages: []MessagesMessage{{Role: "user", Content: json.RawMessage(`"hi"`)}}, Stream: true}
...
func TestNativeMessagesStreamUnknownMultiline(t *testing.T) {
	...
	defer s.Close()
	stream, err := NewClient("key", WithBaseURL(s.URL), WithHTTPClient(s.Client())).MessagesStream(context.Background(), &MessagesRequest{Model: "m"})
	if err != nil {
		t.Fatal(err)
	}
	e, err := stream.Recv()
