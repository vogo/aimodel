...
func TestWithTimeoutBoundsTheCall(t *testing.T) {
	...
	start := time.Now()
	if _, err := client.ChatCompletions(context.Background(), &ChatCompletionRequest{Model: "gpt-5"}); err == nil {
...
func TestWithTimeoutPreservesAnEarlierTransport(t *testing.T) {
...
func TestWithBaseURLTrimsTrailingSlash(t *testing.T) {
	client := NewClient("key", WithBaseURL("https://example.com/v1/"))
